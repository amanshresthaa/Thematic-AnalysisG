...
class KeywordExtractionSignature(dspy.Signature):
    ...
    quotations: List[Dict[str, str]] = dspy.InputField(
        desc="List of quotations containing 'quote' field."
    )
    keywords: List[str] = dspy.OutputField(
        desc="List of extracted keywords."
    )
    ...
    def forward(self, research_objectives: str, quotations: List[Dict[str, str]]) -> Dict[str, List[str]]:
        try:
            ...
            quotes_text = "\n".join([f"Quote {i+1}: {q['quote']}" for i, q in enumerate(quotations)])
            
            prompt = (
                f"You are an expert in qualitative research and thematic analysis.\n\n"
                f"Research Objectives: {research_objectives}\n\n"
                f"Analyze these quotes about potato farming and supply chain:\n{quotes_text}\n\n"
                f"Extract key terms, concepts, and themes from these quotations. "
                ...
                f"and important themes mentioned in the quotes."
            )
...
def save_keywords(keywords: List[str], output_file: str):
    """Save extracted keywords to output JSON file."""
    try:
